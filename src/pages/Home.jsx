...
class Home extends Component {
  ...
  createTodo = async () => {
    ...
    console.log("create todo function >>>>>");
  ...
  render() {
    ...
    const { task, title, isTaskEmpty, isTitleEmpty, status, tags, tag } =
      this.state;
    return (
      <div className="flex flex-col gap-6 max-w-screen-sm mx-auto py-5">
        ...
        {isError && toast.error("Failed to create To-do", 800)}
      </div>
    );
  }
}
...
const mapStateToProps = (state, ownProps) => {
  const { isError, error, isLoading } = state.todo;
